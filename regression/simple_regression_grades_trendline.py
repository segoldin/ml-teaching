...
path_to_file = '/Users/krai/Downloads/ml-teaching-main/regression/programming_grades.csv'
...
y = df['IntroProgramming'].values.reshape(-1, 1)
X = df['DataStructures'].values.reshape(-1, 1)
...
ax.scatter(x = df['IntroProgramming'], y = df['DataStructures'])

# change the pandas dataframe into numpy dataframe
np_df_intro_programming = df['IntroProgramming'].to_numpy()
y = regressor.coef_*np_df_intro_programming + regressor.intercept_
print(y[0])

# plot trendline
ax.plot(df['IntroProgramming'], y[0])
plt.title("Relationship of IntroProgramming Scores to DataStructures Scores")
plt.xlabel("IntroProgramming")
plt.ylabel("DataStructures")
